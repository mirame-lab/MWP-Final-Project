...
//Delete custorder
app.delete('/mbos/custorder/delete/:id', function(req, res) {
    console.log(`/mbos/custorder/delete/${req.params.id}`);
    console.log(req.body);
    ...
    db.collection('itemorders').where('id_custorder', '==', req.params.id).get().then(function(docs) {
        if (!docs.empty) {
            docs.forEach(item => {
                db.collection('itemorders').doc(item.id).delete();
            });

            db.collection('custorders').doc(req.params.id).delete().then(function(docs) {
                result.status = 'success';
                result.message = 'Successful delete custorder from database';
...
//End delete custorder

//Order custorders in sequence
app.get('/mbos/custorders/:orderby/:sequence', function(req, res) {
    var orderby = req.params.orderby;
    var sequence = req.params.sequence;
    var valid_orderby = { 'date_time': 1 };
...
//End order custorders in sequence
